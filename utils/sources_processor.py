...
class SourcesProcessor(Handler):
    def __init__(self, heap_que: list[QueryT], max_queries, sources_speed: list):
        self.sources: list[Source] = [Source(time) for time in sources_speed]
    ...
    def init(self):
        for pos, source in enumerate(self.sources):
            ...
            singleton.stats_collector.add_query(pos)
    ...
    def handle(self, query: QueryT):  # handle query from heap
        if self.total_gen_queries == self.MAX_N_QUERIES:
            ...
            singleton.stats_collector.add_query(query.n_source)
